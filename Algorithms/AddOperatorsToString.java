...
class AddOperatorsToString{
	...
	private long calculate(String e) {
		...
		for (int i = 0; i < e.length(); i++) {
		  	...
		  	if (Character.isDigit(c)) {
			    ...
			    if (!operator.isEmpty() && ((pri[c] < pri[operator.peek()]) 
			    	   || (pri[c] == pri[operator.peek()] && pri[c] == 2))) {
	...
	private long op(long a, long b, Character operator) {
		long result = 0L;
		if (operator == '+') {
		  result =  b + a;. 1point 3acres 璁哄潧
		} else if (operator == '-') {
		  result =  b - a;. 1point3acres.com/bbs
		} else if (operator == '*') {
		  result =  b * a;. 1point 3acres 璁哄潧
		} else if (operator == '/') {
		  if (a == 0) {
