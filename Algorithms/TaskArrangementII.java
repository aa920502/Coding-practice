...
class TaskArrangementII {
	...
	// Then always choose the valid task with highest frequence. 
	...
	public static int getShortestTime(String task, int k) {
		...
		PriorityQueue<Node> queue = 
				new PriorityQueue<Node>(count.size(), Node.NodeComparator);
		...
		while (!queue.isEmpty()) { 
			...
			if (queue.isEmpty()) {
				...
				lastTime.put(cur.letter, ret); //update last time
				if (cur.num != 0) {
					queue.add(cur);
				}
